...
costo(in(arrivato(X),viaggio(Y,C1)),in(arrivato(X),arrivato(Y)),C):-C is C + C1,!.

%Gruppo due arrivato, uno in viaggio.

costo(in(viaggio(X,C1),arrivato(Y)),in(arrivato(X),arrivato(Y)),C):-C is C + C1,!.

%Gruppo uno in viaggio, due in viaggio.
