...
%Gruppo uno arrivato, due in viaggio.

costo(in(arrivato(X),viaggio(Y,C1)),in(arrivato(X),arrivato(Y)),C):-C is C1,!.

%Gruppo due arrivato, uno in viaggio.
...
%primo al sicuro,secondo in incrocio //controllare
costo(in(arrivato(_X),arrivato(Y)),in(arrivato(_Z),arrivato(Q)),C):-connesso(Y,Q,C2),C is C2,!.

%secondo al sicuro,primo in incrocio //controllare
costo(in(arrivato(X),arrivato(_Y)),in(arrivato(Z),arrivato(_Q)),C):-connesso(X,Z,C2),C is C2,!.

%In viaggio il primo gruppo, arriva primo gruppo //controllare
costo(in(viaggio(X,C1),arrivato(Y)),in(arrivato(Z),viaggio(Q,C2)),C):-connesso(X,Z,C1),connesso(Y,Q,C2),C is C2 + C1,!.

%In viaggio il secondo, arriva il primo //controllare
costo(in(arrivato(X),viaggio(Y,C1)),in(arrivato(Z),viaggio(Q,C1)),C):-connesso(X,Z,C2),connesso(Y,Q,C3),C is C2 + (C3-(C2-C1)),!.
