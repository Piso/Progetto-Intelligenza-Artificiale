...
costo(in(X,Z),in(X,Q),C):-traffico(Z,Q), connesso(Z,Q,C1),meteo(_,P),equip(_,P1), C is C1 + 5 + P + P1,!.
costo(in(X,Z),in(Y,Z),C):-traffico(X,Y),connesso(X,Y,C1),meteo(_,P),equip(_,P1),C is C1 + 5 + P + P1,!.
%Qui sotto traffico su entrambe le strade.
costo(in(X,Z),in(Y,Q),C):-traffico(X,Y),traffico(Z,Q),connesso(X,Y,C1),connesso(Z,Q,C2),meteo(_,P),equip(_,P1),C is C1 + C2 + 10 + P + P1,!.
%Qui sotto traffico su una delle due strade.
costo(in(X,Z),in(Y,Q),C):-(traffico(X,Y);traffico(Z,Q)),connesso(X,Y,C1),connesso(Z,Q,C2),meteo(_,P),equip(_,P1),C is C1 + C2 + 5 + P + P + P1 + P1,!.

%costi senza traffico
costo(in(X,Z),in(X,Q),C):-connesso(Z,Q,C1),meteo(_,P),equip(_,P1),C is C1 + P + P1,!.
costo(in(X,Z),in(Y,Z),C):-connesso(X,Y,C1),meteo(_,P),equip(_,P1),C is C1 + P + P1,!.
costo(in(X,Z),in(Y,Q),C):-connesso(X,Y,C1),meteo(_,P),connesso(Z,Q,C2),equip(_,P1),C is C1 + C2 + P + P + P1 + P1.
